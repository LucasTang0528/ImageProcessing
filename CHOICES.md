...
## 8. A comparison that was withdrawn

A second implementation of the E1 and E2 sweeps was written in parallel with
this one and has been discarded. One result from it must not survive, so it is
recorded here rather than left to be rediscovered.

**The withdrawn E1.1 was confounded.** Its colour-histogram comparator did not
apply specular exclusion, while the dominant colour descriptor it was compared
against did. The measured gap therefore mixed the question E1.1 asks - dominant
colours against a histogram - with a second difference nobody intended to test,
specular exclusion on against off. The figure it produced, 0.8833 against
0.8828, is not a measurement of anything the report claims and does not appear
in it. Nothing from that run reaches Chapter 4, and no conclusion about
specular exclusion drawn from it is carried forward.

The surviving comparator holds specular handling constant across both arms,
which is what makes the remaining difference attributable to the descriptor
family. It also uses the skewness of Stricker and Orengo (1995) - the signed
cube root of the mean cubed deviation, in the channel's own units - rather than
the dimensionless Fisher-Pearson ratio, because a colour-moments descriptor
should follow the definition its own literature uses.

Two properties of the withdrawn work were kept, because they change what the
sweeps can conclude rather than how they are computed: the noise band that
decides when two arms are indistinguishable, and the paired fold differences it
is computed from. Their absence is why an earlier reading of the block ablation
called a real 0.37-point effect noise.

**A selection made under the withdrawn comparator was also discarded.** That
sweep's tie band selected a three-colour descriptor. Since it was selected on
fold data produced by the confounded comparator, it is not carried forward as a
prior: whatever the band selects on the surviving sweep is the answer.
