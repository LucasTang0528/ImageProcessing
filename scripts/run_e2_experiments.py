...
#: earns any parsimony advantage through dimensionality instead.
PIPELINE_STAGES = {
    "baseline": 4,
    "distances_1": 4, "distances_123": 4,
    "levels_16": 4, "levels_64": 4,
    "angle_averaged": 4,
    "intensity_moments": 2,
}
...
def e2_1_baseline(
    ...
    for arm, (key, description) in arms.items():
        row = score(matrices[key], arm, config, config_key=key,
                    descriptor=description, **control)
    ...
    return add_paired_differences(pd.DataFrame(rows), "glcm_40")
...
def e2_2_distances(matrices: Dict[str, FeatureMatrix], config: Config) -> pd.DataFrame:
    ...
    return add_paired_differences(pd.DataFrame(
        [
            score(matrices[key], f"distances_{arm}", config, config_key=key,
...
def e2_3_levels(
    ...
    for levels, key in arms.items():
        ...
        rows.append(
            score(
                matrices[key], f"levels_{levels}", config, config_key=key,
                levels=levels,
    ...
    return add_paired_differences(pd.DataFrame(rows), "levels_32")
...
def e2_4_angles(matrices: Dict[str, FeatureMatrix], config: Config) -> pd.DataFrame:
    ...
    return add_paired_differences(pd.DataFrame(
        [
            score(matrices[key], arm, config, config_key=key,
